...
const fetchMockSchema = async () => {
    const importResult = await import('../../test/mock/schema.json', {
        assert: { type: 'json' },
    });
...
const getSchema = async () => {
    ...
    const schema = IS_MOCK ? await fetchMockSchema() : await fetchSchema();
...
const getSchemaByQueryName = (schemaIntrospection, queryName, preciseMatch) => {
    ...
    const fields = _.chain(queries)
        .filter((item) => {
            ...
            return (
                item?.name?.includes(queryName) || item?.description?.includes(queryName)
            );
        })
        .map((item) => {
    ...
    try {
        node.fields?.forEach((field) => {
            ...
            if (fieldType) {
                ...
                if (child) {
                    const childNode = loopFindSchemaTypeChild(
                        schema,
                        {
                            ...child,
