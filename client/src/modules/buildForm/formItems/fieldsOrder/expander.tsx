...
export default function FieldsOrderExpander({schemaList, className, title}: Props) {
    ...
    const columnList = formState?.columnSortNames?.split(/[,\s]+/)?.filter(item => item !== '') ?? [];
    ...
    const content = columnList.map((item) => {
        const matchedSchemas: Array<GraphqlSchema> | undefined = filteredSchemaList.filter((schema: GraphqlSchema) => {
            if (usePreciseMatch) {
    ...
    const findSchemaOnSchemaTree = (schema: GraphqlSchema | undefined, currentKey: string) => {
    ...
    const handleSchemaCheck = (schema: GraphqlSchema) => {
        ...
        setExtraState?.((ownState) => {
            ...
            pathList.forEach(path => {
                ...
                currentSchema = findSchemaOnSchemaTree(currentSchema ?? ownState.schema, currentKey);
            })
            if (currentSchema) {
        ...
        <div className='fields-orders-expander'>
            <div className='fields-list-title'>
                ...
                <div className='operation-list'>
                    <Checkbox value={usePreciseMatch} label='精确查找'
                              onChange={handleUsePreciseMatchChange}/>
                    <Checkbox value={isMatchCheckedSchema} label='匹配已选择节点'
                                    ...
                                    <div className='matched-field-list'>
                                        {
                                            item.matchedSchemas.map(schema => {
                                                return (
                                                    <div className={classNames({'checked': schema.isChecked})}
