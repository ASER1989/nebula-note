...
export default function CustomerTemplate() {
    ...
    const { templateConfig, reloadTemplateConfig, setTemplateKeyword } =
        useTemplateConfig();
    ...
    const expandRender = ({ customerTemplate }: ExtraState) => {
        return (
            <Expander
                templateOption={customerTemplate ?? {}}
    ...
    const handleTemplateTypeChange = (option: DropdownOption) => {
        request.get<string>('/template/content', { path: option.value }).then((resp) => {
            if (!resp.success) {
                return showMessage(
