...
const ResizableBox: React.FC<ResizableBoxProps> = (props) => {
  ...
  const handleCalculateCursorType = (e: MouseEvent) => {
    if (anchor.current && !isResizing.current) {
      ...
      setCursorType(isEwResize ? 'ew-resize' : isNsResize ? 'ns-resize' : 'default');
  ...
  const handleMouseMove = (e: MouseEvent) => {
    if (isResizing.current && anchor.current) {
      ...
      if (isOverLeft || isOverRight) {
        ...
        const widthOffset = e.clientX - anchor.current.getBoundingClientRect()[isOverRight ? 'right' : 'left'];
      ...
      if (isOverTop || isOverBottom) {
        ...
        const heightOffset = e.clientY - anchor.current.getBoundingClientRect()[isOverBottom ? 'bottom' : 'top'];

        const newHeight = anchorHeight - (isOverBottom ? -heightOffset : heightOffset);
